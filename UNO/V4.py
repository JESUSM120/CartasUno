...
while True:
    ...
    if contours:
        ...
        if mejor_coincidencia:
            ...
            cv2.putText(frame, f'Color: {color}, Numero: {numero}', (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            
            # Mostrar la carta detectada al usuario
            cv2.imshow('Carta Detectada', carta_detectada)
            cv2.waitKey(0)
            
            # Solicitar confirmación al usuario
            confirmacion = input("¿Es correcta la identificación de la carta? (s/n): ")
            
            if confirmacion.lower() != 's':
                # Si la identificación no es correcta, permitir al usuario ingresar manualmente el color y el número
                color = input("Ingrese el color correcto de la carta: ")
                numero = input("Ingrese el número correcto de la carta: ")
                
            # Actualizar la identificación de la carta
            mejor_coincidencia = (color, numero)
